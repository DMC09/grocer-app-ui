...
test.describe("Single User Scenarios", () => {
  test("Main Workflow", async ({ page }) => {
    ...
    await test.step("Update item notes to be sharp cheddar and quantity to 2 and add category", async () => {
    ...
    await test.step("Verify Cheese, Eggs and Thighs items ", async () => {
      await page.getByRole("button", { name: "Diary" });
    });

    await test.step("Verify Dairy filter work ", async () => {
      await page.getByRole("button", { name: "Diary" });
      // Verify only Cheese Shows
    });

    await test.step("Add common item Eggs  ", async () => {});

    await test.step("Verify Dairy filter wwork ", async () => {});
